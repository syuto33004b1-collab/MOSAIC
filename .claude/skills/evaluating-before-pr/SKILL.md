...
完了主張を、**その差分を書いた者とは別のモデル**に独立検証させる。**評価者は助言専用**で、ファイル変更もコマンド実行もさせない。誰が評価者になるかは差分を書いた者で決まる。[評価者](#評価者)を先に読む。
...
**評価者は、その差分を実装した者と別のモデルにする。** ここが規則で、以下は今いる実装者への当てはめにすぎない。

判定は git の author では行わない。コミットは人の名前で残る。問うのは「**この呼び出しがその差分を書いたか**」である。

| その差分を書いたのは | 評価者 | 手順 |
| --- | --- | --- |
| この呼び出し（Claude） | codex の `gpt-5.6-sol` | 以下すべて。**走ったことがある唯一の経路** |
| この呼び出しではない（codex など） | Claude | **起動手順がまだ無い。下記「自分が書いていない差分」へ** |

エフォートは実装が Claude の場合。
...
**この呼び出しでは評価しない。** 自分が書いていないので自己レビューにはならないが、起動手順が無い。

- **PR を作らない。** 利用者へ「この差分は自分が書いたものではなく、評価の起動手順がまだ無い」と伝え、判断を仰ぐ
- 起動手順は #320 の実測のあとに書く。**未走行の手順をここへ書き足さない**

### 混在しているとき

同じ PR に、この呼び出しが書いた領域と、書いていない領域がある場合。

- その領域を書いていない側が、その領域を評価する
- 分けられないなら止めて利用者に聞く
...
- 指摘ゼロでも、評価記録（`評価者:` / `エフォート:` / `評価対象コミット:`）を PR 本文へ書く。**書式と CI の検証内容は AGENTS.md の 12。** 記録が無いと `Evaluation record` チェックが落ちる

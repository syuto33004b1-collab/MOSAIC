...
資格はリクエストを発行者へ紐付けてから既存の RPC を再利用するため、**そのとき紐付いた利用者**のロールに設定されたロール別権限（`app.role_permissions`）が適用されます。スコープを満たしていても、非表示の独自項目、利用不可の機能、参照範囲外の人には届きません。
...
> **既知の問題（[#54](https://github.com/syuto33004b1-collab/MOSAIC/issues/54)）**
> `private.become_integration_actor` は、発行者が active な owner / admin / planner のいずれでもなくなったとき、組織内の任意の owner または admin へ紐付けをフォールバックします。発行者を降格・利用停止しても資格は `active` のままなので、**適用されるロール別権限が制限の緩い側へ意図せず切り替わります**。発行者が実行ロールを失った資格は停止させる方向で #54 で修正します。それまでは、資格を発行した利用者の権限変更時に、対応する資格を運用パネルで失効させてください。

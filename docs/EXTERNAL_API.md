...
資格は**発行した利用者としてだけ**動きます。発行者が active な owner / admin / planner のいずれでもなくなると、資格は `active` のままでも `401` で停止します。別の管理者へ昇格することはありません。復旧は運用パネルで失効させ、再発行してください。

停止した資格は運用パネルの一覧に「停止中（発行者が権限を失いました）」と表示されます。停止理由は資格の保有者へは返しません（`invalid credential` のみ）。

...
確認の強度はチャットの確認カードと同等ではありません。ホストが自動承認する場合、モデルが2回目を連続で呼べます。サーバは人間の承認を検証できません。2往復が買うのは、本文が書込み前にトランスクリプトへ露出すること、単発の幻覚呼び出しだけでは書けないこと、ホストが承認の機会を2回持てること（ホスト実装次第）です。最初の書込面をフィードバックにした理由でもあります。

...
- owner/adminが保留中の招待を一覧・取消でき、停止済み利用者は再招待・再受諾で復帰できない。
...
- 新規利用者はAuth userを先に管理された経路で作成し、MOSAICの組織招待を同じメールへ登録します。招待メールはMOSAICから自動送信されません。
- 誤招待は運用パネルの「保留中の招待」から即時取消し、期限切れを放置しません。

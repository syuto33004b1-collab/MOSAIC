...
評価は機械的検証の**代替ではない**。先に通し、その結果を評価者へ渡す。

1. **変更を全てコミットする。** 評価対象は `git diff origin/main...HEAD` であり、未コミット・未ステージの変更は評価から漏れる。`git status --short` が空であることを確認する
2. **機械的検証を通す**（コマンドは AGENTS.md）
3. **UI に影響するなら `verifying-ui-changes` を通す**

### 評価と取り込み

1. 評価者を1回起動する（既定は Sol）
2. Critical / High は直す。Medium / Note は採用・保留・反論を明示する
3. 直したあと、**機械的検証と UI 確認はやり直す。** 修正が別の壊れを生んでいないかを未検証にしない
4. **2回目の評価はしない。** PRを出す
5. ただし指摘対応**以外**の変更を加えたなら、評価対象が変わっているので再評価する

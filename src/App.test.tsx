...
describe("a key for what colour and position encode", () => {
  ...
  it("puts every week of the staffing rail into its accessible name", async () => {
    ...
    const rail = document.querySelector(".four-week-rail")!;
  ...
  it("puts every proficiency level into the rail's accessible name", async () => {
    ...
    const rail = document.querySelector(".proficiency-rail")!;
    const label = rail.getAttribute("aria-label") ?? "";
  ...
  it("gives each visualisation a key immediately above its table", async () => {
    ...
    expect(projectCaption.textContent).toContain("今週から4週分");
    expect(projectCaption.nextElementSibling).toBe(document.querySelector(".portfolio-table-wrap"));
    ...
    expect(skillCaption.nextElementSibling).toBe(document.querySelector(".skill-map-wrap"));
  });
  ...
  it("tells the 未充足 links apart, and the count badges apart", async () => {
    ...
    // The two sidebar badges count different things, so they say which.
    const navigation = within(screen.getByRole("navigation", { name: "メインナビゲーション" }));
    ...
    expect(projects.querySelector(".nav-count")).toHaveAttribute("title", expect.stringMatching(/^登録\d+件$/u));
    expect(opportunities.querySelector(".nav-count")).toHaveAttribute("title", expect.stringMatching(/^進行中\d+件$/u));
    expect(projects.getAttribute("aria-label")).not.toBe(opportunities.getAttribute("aria-label"));
  });
});

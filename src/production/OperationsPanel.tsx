...
import { Check, Clock3, History, KeyRound, LogOut, MailPlus, MailX, MessageSquarePlus, Plug, Radio, RefreshCw, ShieldCheck, UsersRound, X } from "lucide-react";
...
export function OperationsPanel({
  ...
  const [composeRequest, setComposeRequest] = useState<{ body: string; requestId: string } | null>(null);
  ...
  const submitComposeFeedback = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const body = composeBody.trim();
    if (!body || composeSending) return;
    const request = composeRequest?.body === body
      ? composeRequest
    ...
    try {
      ...
      if (canViewFeedback) await loadOperations();
    } catch (reason) {
      setError(messageFrom(reason));
    } finally {
  ...
  const navItems: { id: SettingsSection; label: string; show: boolean }[] = [
    { id: "members", label: "メンバー", show: true },
    { id: "invite", label: "招待", show: canInvite },
    { id: "integrations", label: "外部連携", show: canInvite },
    { id: "feedback-admin", label: "気づき", show: canViewFeedback },
    { id: "audit", label: "監査ログ", show: canViewAudit },
    { id: "feedback-send", label: "気づきを送る", show: true },
    { id: "legal", label: "規約", show: true },
    { id: "signout", label: "退出", show: Boolean(onSignOut) },
  ];

  const visibleNav = navItems.filter((item) => item.show);
  const activeSection = visibleNav.some((item) => item.id === section) ? section : "members";

  return (
    <div className="overlay production-operations-overlay">
      <button className="overlay-backdrop" aria-label="設定を閉じる" onClick={onClose} />
      <section
      ...
      >
        <div className="drawer-top">
          ...
          <button className="close-button" aria-label="設定を閉じる" onClick={onClose}><X size={18} /></button>
        </div>

        <div className="drawer-heading">
          <span className="drawer-icon cobalt"><ShieldCheck size={19} /></span>
          <div><h2 id="operations-title">設定</h2><p>{currentOrganization.name} · {currentOrganization.role}</p></div>
        </div>
        ...
        <div className="production-settings-panes">
          <nav className="production-settings-nav" aria-label="設定カテゴリ">
            {visibleNav.map((item) => (
              <button
                key={item.id}
                type="button"
                className="production-settings-nav-item"
          ...
          <div className="production-settings-detail">
            ...
            {activeSection === "invite" && canInvite && (
              <>
            ...
            {activeSection === "integrations" && canInvite && (
              <>
            ...
            {activeSection === "feedback-admin" && canViewFeedback && (
              <>
            ...
            {activeSection === "feedback-send" && (
              <>
                ...
                <form className="assignment-form production-invite-form" onSubmit={(event) => void submitComposeFeedback(event)}>
                  ...
                  <button className="drawer-primary" type="submit" disabled={composeSending || !composeBody.trim()}>
                    <MessageSquarePlus size={15} />{composeSending ? "送信中…" : "送る"}
                  </button>
                </form>
              </>
            )}
            ...
            {activeSection === "legal" && (
              <>
                <div className="drawer-section-title"><span>規約</span></div>
                <p className="production-settings-lead">プライバシーポリシーと利用規約を確認できます。</p>
                <a className="drawer-secondary production-settings-link" href={withLegalSearch(window.location)}>プライバシーポリシーと利用規約</a>
              </>
            )}

            {activeSection === "signout" && onSignOut && (
              <>
                <div className="drawer-section-title"><span>退出</span></div>
                <p className="production-settings-lead">このアカウントからログアウトします。未保存の変更がある場合は、設定を開く前に確認しています。</p>
                <button className="drawer-secondary production-sign-out" type="button" onClick={onSignOut}>
